...
The spec file is **NOT** a planning artifact — it does not follow `kind:` / `name:` convention from `.codex/skills/plan-pipeline/SKILL.md`. Specs are test contracts, parallel to `library-refs.md` exemption.
...
- **plan-pipeline overview** — `/plan-test-specs` é Stage 5 (linked back from `.codex/skills/plan-pipeline/SKILL.md`).
