...
description: >
  ...
  multi-file skill at `.codex/skills/testing-guide-<project>/` with main SKILL.md and artifact/reference sub-files. Invoke with `/generate-test-guide <project-folder>`.
...
**Output:** a multi-file skill at `.codex/skills/testing-guide-<project>/` (e.g., `testing-guide-nestjs-project`) with a main `SKILL.md` (~200-250 lines) and detailed guides in `artifacts/` and `references/` subdirectories. Each project gets its own skill, enabling monorepo support.

---

## Phase 1 — Project Analysis

First, read `.codex/skills/generate-test-guide/testing-fundamentals.md` — this contains universal testing principles that serve as the foundation. Having these loaded early allows you to filter web research results against them in Phase 2.
...
- Check if `.codex/skills/` already has testing-related skills (e.g., `test-guide`).
...
The skill directory and name are derived from the project folder name: `.codex/skills/testing-guide-<project>/`.

For example, if `$ARGUMENTS` is `nestjs-project`, the output directory is `.codex/skills/testing-guide-nestjs-project/`. This allows multiple project-specific testing guides to coexist in a monorepo.
...
```
.codex/skills/testing-guide-<project>/
...
- [ ] No contradiction with existing skills in `.codex/skills/` or rules in `.claude/rules/`
...
1. Create the skill directory `.codex/skills/testing-guide-<project>/` (if it doesn't already exist)
...
- **Do not modify** any existing files in `.codex/skills/` other than the generated `.codex/skills/testing-guide-<project>/` directory.
